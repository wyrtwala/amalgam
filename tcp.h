...
int tcp_send_ipv4(char* ip, int port, char* buffer, uint_64 buffer_size){}
  ...
  if (connect(socket, (struct sockaddr *) &address, sizeof(address)) != 0){
    fprintf(stderr, "TCP_SEND_IPV4: Failed to connect to %s:%d\n", ip, port);
    return -1;
  }
  uint64_t message_size = buffersize - 1;
  if (recv(socket, buffer, message_size, 0) != 0){
    fprintf(stderr, "TCP_SEND_IPV4: Failed to recieve from %s:%d\n", ip, port);
  }
  if (close(socket) != 0){
...
int tcp_simple_server_ipv4(char* ip, int port, char* message, uint64_t message_size, uint64_t listen_count){
    ...
    if (bind(server_socket, (struct sockaddr *) &address, sizeof(address)) != 0){
      fprintf(stderr, "TCP_SIMPLE_SERVER_IPV4: Failed to bind at port %d\n", port);
      return -1;
    }
